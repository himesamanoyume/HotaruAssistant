...
- ~~支援角色似乎没正常使用~~
- 通过文字识别 得到可领取奖励的最左侧的任务名称，领取之后给对应任务改为false
- 实现saveConfigByUid
- 模拟宇宙界面识别积分和沉浸器数量，放积分为0时，自动打完一次然后领取7500，再打第二次且如果有沉浸器则同时领取沉浸奖励，出来再领取15000，如果积分为15000后则需要根据配置文件是否开启领取沉浸决定是否领取，否则需要运行时就只单纯打完
- - 细化模拟宇宙配置，展示更加详细的运行情况，如记录第一次第二次通关模拟宇宙的时间戳，根据判断，决定进行几次模拟宇宙，同时完成要求
- 模拟宇宙通关后即使设置了领取沉浸奖励依然不领取
- ui支持多账号显示
- - 拿到uid后应该对所有需要的config配置项中添加对应uid的分支
- - **针对多账号存储不同的配置和记录（多账号配置添加设定清体力选择的副本，并能在GUI当中直接设置）**
