...
- 发送邮件时把该uid的配置列出
- notify时先初始化所有用到的内容再根据情况赋值
- 侵蚀隧洞连续挑战逻辑bug
- ~~新增临时黑名单,检测到是config中黑名单的uid时跳过启动~~
- 加入顶号处理 掉线处理
- ~~config新增记录当前运行到的uid 方便重开,get_new_uid中~~
- 通知邮件加上头图
- ~~优化:当识别到是实训界面时,都先进行一遍~~
- 新增一个配置项,每次启动新号时,如果该配置项里有新的注册表地址,则加入到for循环尾部(want_add_accounts)
