...
class ScreenBaseMgr:
    ...
    def __init__(self, log, logMgr, configPath="./assets/config/screens.json"):
    ...
    def ShowDetectArea(self, detectArea, duration = 3):
        if self.mDevScreen.isDevScreenRunning:
            try:
                if detectArea == (0,0,0,0):
                    return
                
                self.mDevScreen.ShowDetectArea(detectArea, duration)
            except Exception as e:
                self.log.error(self.logMgr.Error(f"canvas删除元素失败:{e}"))
    ...
    def CheckResulotion(self, title, width, height):
        ...
        if w != width or h != height:
            self.log.error(self.logMgr.Error(f"游戏分辨率 {w}*{h} 请在游戏设置内切换为 {width}*{height} 窗口或全屏运行"))
            input("按回车键关闭窗口. . .")
            sys.exit(1)
        else:
            self.log.debug(self.logMgr.Debug(f"游戏分辨率 {w}*{h}"))
    ...
    def PerformOperations(self, operations):
        ...
        for operation in operations:
            ...
            if hasattr(self, functionName):
                ...
                # self.log.info(self.logMgr.Info(f"执行了一个操作: {func}"))
            else:
                ...
                if module and hasattr(module, methodName):
                    ...
                    # self.log.info(self.logMgr.Info(f"执行了一个操作: {functionName}"))
                else:
                    self.log.warning(self.logMgr.Warning(f"未知的操作: {functionName}"))
    ...
    def FindScreen(self, screenName, screen):
            try:
                ...
                self.log.debug(self.logMgr.Debug(f"识别界面出错：{e}"))
    ...
    def GetCurrentScreen(self, autotry=True, maxRetries=5):
        ...
        for i in range(maxRetries):
            ...
            if self.currentScreen:
                ...
                self.log.info(self.logMgr.Info(f"当前界面：{screenName}"))
                return True
            ...
            if autotry:
                ...
                break
        self.log.error(self.logMgr.Error("当前界面：未知"))
        return False
    ...
    def ChangeTo(self, targetScreen, maxRecursion=2):
        ...
        if self.CheckScreen(targetScreen):
            self.log.debug(self.logMgr.Debug(f"已经在 {self.GetName(targetScreen)} 界面"))
            return
        ...
        if path:
            for i in range(len(path) - 1):
                ...
                if self.currentScreen != nextScreen:
                    if maxRecursion > 0:
                        self.log.warning(self.logMgr.Warning(f"切换到 {self.GetName(nextScreen)} 超时，准备重试"))
                        self.ChangeTo(nextScreen, maxRecursion=maxRecursion - 1)
                    else:
                        nowtime = time.time()
                        self.log.error(self.logMgr.Error(f"{nowtime},无法切换到 {self.GetName(nextScreen)},请确保你的账号已经解锁该功能,且不要在配置中选择你未解锁的副本或功能"))
