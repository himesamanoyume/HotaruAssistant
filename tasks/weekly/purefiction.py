...
class PureFiction:
    def get_star_and_level():
        ...
        if auto.click_element("虚构叙事", "text", max_retries=20, crop=guide4_little_crop):
            time.sleep(1)
            ...
            try:
                ...
                logger.info(gu(f"虚构叙事刷新倒计时:{countdownText},层数:{levelText},星数:{starText}"))
                Utils._content['pf_countdownText'] = countdownText
                level = levelText.split('/')[0]
                star = starText.split('/')[0]
                config.purefiction_levels[Utils.get_uid()] = int(level)
                config.purefiction_stars[Utils.get_uid()] = int(star)
                config.save_config()
            except Exception as e:
                nowtime = time.time()
                logger.error(gu(f"{nowtime},识别虚构叙事失败:{e}"))
