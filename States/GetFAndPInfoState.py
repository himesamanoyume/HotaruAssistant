...
class GetFAndPInfoState(BaseState):
    ...
    def GetForgottenHallInfo():
        ...
        if screenMgr.ClickElement("忘却之庭", "text", maxRetries=20, crop=guide4_little_crop):
            ...
            try:
                time.sleep(0.5)
                countdownText = screenMgr.GetSingleLineText(crop=countdownTextCrop, blacklist=[], maxRetries=6)
                countdownText = countdownText.replace('）','').replace(')','').replace('①','').replace('?','')
                if countdownText == '?':
                    countdownText = '识别出错'
                levelText = screenMgr.GetSingleLineText(crop=levelTextCrop, blacklist=[], maxRetries=3)
                ...
                configMgr.mConfig[configMgr.mKey.FORGOTTENHALL_STARS][dataMgr.currentUid] = int(star)
    ...
    def GetPureFictionInfo():
        ...
        if screenMgr.ClickElement("虚构叙事", "text", maxRetries=20, crop=guide4_little_crop):
            ...
            try:
                ...
                configMgr.mConfig[configMgr.mKey.PUREFICTION_STARS][dataMgr.currentUid] = int(star)
            except Exception as e:
                nowtime = time.time()
