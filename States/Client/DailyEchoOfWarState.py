...
class DailyEchoOfWarState(BaseFightState, BaseClientState):
    ...
    def EchoOfWarGetTimes():
        ...
        if screenClientMgr.ClickElement("侵蚀隧洞", "text", maxRetries=10, crop=guide3Crop):
            screenClientMgr.MouseScroll(12, -1)
            time.sleep(1)
            if screenClientMgr.ClickElement("历战余响", "text", maxRetries=10, crop=guide3Crop):
                screenClientMgr.FindElement("历战余响", "text", maxRetries=10, crop=(
