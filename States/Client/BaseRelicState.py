...
class BaseRelicState(BaseClientState):
    ...
    def InstanceGetRelic():
        ...
        point = screenClientMgr.FindElement("./assets/images/fight/fight_reward.png", "image", 0.9, maxRetries=2)
    ...
    def CreateRelicContent(relicName, relicPart, relicList):
        ...
        time.sleep(1)
        if screenClientMgr.ClickElement("./assets/images/fight/relic_lock.png", "image", 0.9, maxRetries=5):
    ...
    def GarbageRelic():
        ...
        if screenClientMgr.ClickElement("./assets/images/fight/relic_rubbish.png", "image", 0.9, maxRetries=5):
    ...
    def SalvageRelics():
        try:
            ...
            if not configMgr.mConfig[configMgr.mKey.RELIC_SALVAGE_ENABLE][dataClientMgr.currentUid]:
    ...
    def DetectRelicCount():
        try:
            ...
            relicCountText = screenClientMgr.GetSingleLineText(relicCountCrop, ['遗','器','数','量'], maxRetries=5)
            relicCountText = relicCountText.replace('量','')
            log.info(logMgr.Info(f"遗器数量:{relicCountText}"))
            relicCountText = relicCountText.split('/')[0]
            dataClientMgr.currentRelicCount = int(relicCountText)
            
            dataClientMgr.notifyContent["遗器数量"] = dataClientMgr.currentRelicCount
            if dataClientMgr.currentRelicCount >= configMgr.mConfig[configMgr.mKey.RELIC_THRESHOLD_COUNT][dataClientMgr.currentUid]:
    ...
    def SkipForRelicCount():
        if dataClientMgr.currentRelicCount >= configMgr.mConfig[configMgr.mKey.RELIC_THRESHOLD_COUNT][dataClientMgr.currentUid]:
            nowtime = time.time()
            log.error(logMgr.Error(f"{nowtime},检测到遗器数量超过{configMgr.mConfig[configMgr.mKey.RELIC_THRESHOLD_COUNT][dataClientMgr.currentUid]},所有可能获得遗器的副本全部跳过,出现该致命错误意味着你没有选择开启遗器自动分解开关,若不打算开启,则只能自行上号清理,否则每次上号时遗器数量超标时都会直接中止"))
            raise Exception(f"{nowtime},检测到遗器数量超过{configMgr.mConfig[configMgr.mKey.RELIC_THRESHOLD_COUNT][dataClientMgr.currentUid]},所有可能获得遗器的副本全部跳过,出现该致命错误意味着你没有选择开启遗器自动分解开关,若不打算开启,则只能自行上号清理,否则每次上号时遗器数量超标时都会直接中止")