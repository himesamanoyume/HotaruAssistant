        ...
        for i in range(max_retries):
            ...
            if find_type in ['image', 'text', "min_distance_text"]:
                if find_type == 'image':
                    ...
                    top_left, bottom_right = self.find_min_distance_text_element(target, source, source_type, include, need_ocr)
    ...
    def find_min_distance_text_element(self, target, source, source_type, include, need_ocr=True):
        ...
        for box in self.ocr_result:
            ...
            if ((include is None or not include) and text in target) or (include and any(t in text for t in target)):
                ...
                logger.debug(_("目标文字：{target} 相似度：{max_val} 距离：{min_distance}").format(target=matched_text, max_val=box[1][1], min_distance=distance))
        ...
        if target_pos is None:
            logger.debug(_("目标文字：{target} 未找到，没有识别出匹配文字").format(target=", ".join(target)))
            return None, None
        logger.debug(_("目标文字：{target} 最短距离：{min_distance}").format(target=min_target, min_distance=min_distance))
