...
class Notify:
    ...
    def _send_notification_by_smtp(self, title, content):
        ...
        account_active_content = ("<blockquote><p>" if not config.account_active[uid]['ActiveDay'] <= 3 else "<blockquote style=background-color:#5f4040><p>")+f"激活天数剩余:{config.account_active[uid]['ActiveDay']}天</p><p>过期时间:{str(datetime.fromtimestamp(time.time())).split('.')[0]}</p></blockquote>"
